...
function AddFaculty({ isOpen, onClose, onAddFaculty }) {
  ...
  const handleSubmit = (e) => {
    ...
    const newFaculty = {
      ...
      joinDate: `Since ${new Date(formData.joinDate).toLocaleDateString('en-US', { month: 'short', year: 'numeric' })}`,
      courses: formData.courses,
      profileCompletion: Math.floor(Math.random() * 20) + 80, // Random completion between 80-100%
      availability: formData.availability,
      status: formData.status
    };
    
    // Pass the new faculty to parent component
    if (onAddFaculty) {
  ...
  return (
    <div className="w-[800px] max-w-[90vw] bg-white rounded-[12px] p-8 relative">
      ...
      <div className="flex justify-between items-center mb-8">
        <h2 className="text-[24px] font-semibold text-[#1F1D1D]">Add New Faculty</h2>
