...
function AddCourse({ isOpen, onClose, onAddCourse }) {
  ...
  const handleSubmit = (e) => {
    ...
    // Pass the new course to parent component
    if (onAddCourse) {
  ...
  return (
    <div className="w-[720px] max-w-[90vw] h-auto rounded-[8px] bg-[#FFFFFF] p-[32px] max-sm:p-[20px] gap-[10px] flex flex-col relative shadow-lg  overflow-y-auto">
      <button onClick={onClose} className="absolute top-6 right-6 max-sm:top-4 max-sm:right-4 text-gray-500 hover:text-gray-700">
        <X size={20} />
      </button>

      <div className="w-full flex flex-col gap-[24px]">
        <h2 className="font-[Inter] font-semibold text-[20px] text-[#1F1D1D]">Add New Course</h2>

        <form onSubmit={handleSubmit} className="flex flex-col gap-[24px]">
          {/* Course Name */}
          <div className="flex flex-col gap-[8px]">
            <label className="font-[Inter] text-[14px] text-[#717171]">Course Name</label>
            <input
              type="text"
              name="courseName"
          ...
          {/* Course Code */}
          <div className="flex flex-col gap-[8px]">
            <label className="font-[Inter] text-[14px] text-[#717171]">Course Code</label>
            <input
              type="text"
              name="courseCode"
          ...
          {/* Duration */}
          <div className="flex flex-col gap-[8px]">
            <label className="font-[Inter] text-[14px] text-[#717171]">Duration (Years)</label>
            <div className="flex flex-col gap-[8px]">
              <div className="flex items-center">
                ...
                <input
                  ...
                  onChange={(e) => setFormData({...formData, duration: Math.max(1, Math.min(6, parseInt(e.target.value) || 1))})}
                  min="1"
                  max="6"
              ...
              <span className="font-[Inter] text-[14px] text-[#717171]">Only whole years</span>
            </div>
          </div>

          {/* Total Fees */}
          <div className="flex flex-col gap-[8px]">
            <label className="font-[Inter] text-[14px] text-[#717171]">Total Fees (₹)</label>
            <input
              type="number"
              name="totalFees"
          ...
          {/* Description */}
          <div className="flex flex-col gap-[8px]">
            <label className="font-[Inter] text-[14px] text-[#717171]">Description</label>
            <textarea
              name="description"
          ...
          <div className="flex flex-col gap-[8px]">
            <label className="font-[Inter] font-semibold text-[14px] leading-[20px] tracking-normal text-[#1F1D1D]">Semester System</label>
            <div className="flex items-center justify-between">
              <span className="font-[Inter] text-[14px] text-[#717171]">Enable if course follows semester pattern</span>
              <button
                type="button"
              ...
              >
                <div
                  className={`absolute top-[2px] w-[20px] h-[20px] bg-white rounded-full transition-transform ${
                    formData.semesterSystem ? 'translate-x-[26px]' : 'translate-x-[2px]'
                  }`}
                />
              </button>
            </div>
          ...
          <div className="flex flex-col gap-[8px]">
            <label className="font-[Inter] font-semibold text-[14px] leading-[20px] tracking-normal text-[#1F1D1D]">Active Status</label>
            <div className="flex items-center justify-between">
              <span className="font-[Inter] text-[14px] text-[#717171]">Inactive courses won't appear in student registration</span>
              <button
                type="button"
              ...
              >
                <div
                  className={`absolute top-[2px] w-[20px] h-[20px] bg-white rounded-full transition-transform ${
                    formData.activeStatus ? 'translate-x-[26px]' : 'translate-x-[2px]'
                  }`}
                />
              </button>
            </div>
