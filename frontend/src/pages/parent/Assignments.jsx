  ...
  return (
    <>
      <div className="p-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {cards.map((card, idx) => (
            <div
              key={idx}
              className="bg-white rounded-xl shadow p-8 flex flex-col gap-2"
            >
              <div className="flex justify-between items-center">
                <p className="text-sm font-[Inter] text-[#1F1D1D]">
                  {card.title}
                </p>
                {card.icon}
              </div>
              <h2 className="text-2xl font-[Inter]">{card.value}</h2>
              <p className="text-sm text-[#1F1D1D]">{card.subtitle}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="p-5 space-y-8">
        <div className="bg-[#0b2447] text-white font-semibold text-xl px-4 py-3 rounded-xl shadow-md mb-6">
          Assignments
        </div>
          ...
          {submissions.map((sub, idx) => (
            ...
            >
              ...
              <div className="flex justify-between items-start">
                <div>
                  <h3 className="text-base  text-[#1F1D1D]">{sub.title}</h3>
                  <p className="text-sm text-[#1F1D1D]">{sub.subject}</p>
                  <p className="text-sm text-[#1F1D1D]">{sub.desc}</p>
                </div>
              ...
              <div className="flex items-center gap-2 text-sm mt-3">
                <Clock
                  className={`w-4 h-4 ${
                    sub.status === "Overdue" ? "text-red-500" : "text-[#1F1D1D]"
                  }`}
                />
                <span
                  className={`${
                ...
                <span className="ml-2 text-[#1F1D1D]">
                  Max Marks: {sub.marks}
                </span>
              </div>
              ...
              {sub.extra && (
                <p
                  className={`mt-3 px-3 py-2 text-sm rounded-md flex items-center gap-1 ${sub.extraColor}`}
                >
          ...
          {grades.map((item, idx) => (
            ...
            >
              ...
              <div className="w-full">
                <h3 className="text-lg font-semibold text-[#1F1D1D]">
                  {item.title}
                </h3>
