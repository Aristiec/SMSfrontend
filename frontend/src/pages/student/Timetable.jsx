...
const TimetablePage = () => {
  ...
  const allSlots = [...timeSlots, "BREAK", ...timeSlots2, "LUNCH", ...timeSlots3];
    ...
    <div className="bg-[#E9EEF4] text-[#1F1D1D] font-[Inter] min-h-screen">
      ...
      <div className="p-4 hidden md:block">
        <div className="w-full overflow-x-auto mx-auto">
          <div className="inline-block min-w-full bg-[#FAFCFD] rounded-lg shadow-md border border-white p-4 justify-center">
            <div className="flex flex-nowrap w-full mx-auto">
              {/* === Table 1 === */}
              <table className="border-collapse">
                <thead>
                  <tr>
                    ...
                    {timeSlots.map((time, index) => {
                      ...
                      return (
                        <th key={index} className="text-xs md:text-sm font-normal text-center border border-[#04203E] min-w-[7.9rem] h-16">
                          {start} -<br />{end}
                        </th>
                      );
                    })}
                ...
                <tbody>
                  {days.map((day, dayIndex) => {
                    ...
                    return (
                      <tr key={dayIndex} className={isSelected ? "bg-[#CFDCEB]" : ""}>
                        <td className="font-bold text-center text-xs md:text-sm border border-[#04203E] min-w-[7.9rem] h-16">
                          {day}
                        </td>
                        {timeSlots.map((_, timeIndex) => (
                          <td key={timeIndex} className="border border-[#04203E] min-w-[7.9rem] h-20">
                            <div className="flex flex-col items-center justify-center h-full px-1">
                              <div className="text-xs md:text-sm">Subject</div>
                              <div className="text-[10px] text-[#04203E]">(Prof name)</div>
                            </div>
                          </td>
                        ))}
                      </tr>
              ...
              {/* === Break === */}
              <div className="flex flex-col justify-center items-center px-4 text-[#1F1D1D] font-bold text-sm md:text-lg tracking-wide min-w-[4rem] h-full gap-20 py-16 border">
              ...
              <div className="flex flex-col justify-center gap-20 py-16 items-center px-4 text-[#1F1D1D] font-bold text-sm md:text-lg tracking-wide min-w-[4rem] border h-full">
                <p>L</p><p>U</p><p>N</p><p>C</p><p>H</p>
              </div>
              <table className="border-collapse">
                <thead>
                  <tr>
      ...
      <div className="block md:hidden p-4">
        {days.map((day, dayIndex) => {
          ...
          return (
            <div key={day} className={`mb-6 border border-white rounded-lg bg-white shadow-md p-2 ${isToday ? "bg-[#CFDCEB]" : ""}`}>
                ...
                <tbody>
                  {allSlots.map((slot, idx) => {
                    if (slot === "BREAK" || slot === "LUNCH") {
                      return (
                        <tr key={idx}>
                          <td colSpan={2} className="text-center text-[#04203E] font-medium py-2">
                            --- {slot} ---
                          </td>
                        </tr>
                      );
                    ...
                    return (
                      <tr key={idx}>
                        <td className="border border-[#04203E] p-2 text-center">{slot}</td>
                        <td className="border border-[#04203E] p-2 text-center">
                          <div className="font-medium">Subject</div>
                          <div className="text-[11px] text-[#04203E]">(Prof name)</div>
                        </td>
                      </tr>
                    );
                  })}
