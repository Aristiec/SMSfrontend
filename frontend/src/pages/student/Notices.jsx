...
import NoticeSkeleton from "./LoadingSkeleton/NoticeSkeleton"
...
const Notices = () => {
  const [noticesData, setNoticesData] = useState(null); // default null for loading state
  ...
  useEffect(() => {
    const fetchNotices = async () => {
      try {
        ...
        // If no studentId or API fails, use mock data
        if (!studentId) {
      ...
      <div className="flex flex-col px-4 gap-5 mt-4">
        ...
        <div className="mt-5 space-y-6">
          {noticesData === null ? (
            ...
            <div className="flex justify-center items-center min-h-[calc(100vh-30vh)] bg-gray-50">
              <div className="w-[90%] max-w-4xl bg-white rounded-2xl shadow-lg p-10 flex flex-col items-center gap-6 border border-gray-100">

                {/* Icon Container */}
                <div className="w-[80px] h-[80px] rounded-full bg-[#F5FAFF] flex items-center justify-center shadow-md">
                  <FlagOff size={42} color="#2196F3" />
                </div>

                {/* Title */}
                <h2 className="font-inter font-semibold text-xl text-gray-800 text-center">
            ...
            noticesData.map((notice) => {
              ...
              return (
                <div
                  key={notice.id}
                  className={`${notice.bgColor || "bg-[#FAFCFD]"} shadow-md rounded-[12px] p-6 flex flex-col gap-6`}
                ...
                >
                  ...
                  <div className="flex flex-col gap-3">
                    <h3 className="text-[#1F1D1D] text-lg md:text-xl font-semibold font-[Inter]">
                      {notice.title}
                    </h3>

                    <div
                      className={`overflow-hidden transition-all duration-800 ease-in-out ${isExpanded
                          ? "max-h-[1000px] opacity-100"
                          : "max-h-[72px] opacity-90"
                        }`}
                    >
                      <p className="text-[#1F1D1D] text-base leading-[24px] font-normal font-[Inter] text-justify">
                  ...
                  <div className="flex justify-between items-center">
                    <div className="flex gap-1">
                      <span className="text-[#1F1D1D] text-xs font-light">
                        Posted by:
                      </span>
                      <span className="text-[#1F1D1D] text-xs font-light">
                        {notice.postedBy}
