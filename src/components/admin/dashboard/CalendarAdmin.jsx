...
const CalendarAdmin = () => {
  const [currentDate, setCurrentDate] = useState(new Date(2025, 4, 10));
  ...
  const upcomingEvents = [
    {
      time: "11:30-12:15 PM",
      event: "Tech. Fest",
      staff: "Prof. Rajashekar",
    },
    {
      time: "01:15-02:00 PM",
      event: "Mechanical Workshop",
      staff: "Prof. Nirmala",
    },
  ];
  ...
  return (
    <div className="w-full bg-[#FAFCFD] rounded-2xl shadow-lg overflow-hidden flex flex-col h-[476px]">
      ...
      <div className="px-6 py-4 border-t border-gray-100 flex-grow">
        <h3 className="text-[16px] font-[Inter] font-bold text-[#1F1D1D] mb-5 inline-block border-b-2 border-[#1F1D1D] leading-none">
          Upcoming Events
        </h3>
        ...
        <div className="space-y-4">
          {upcomingEvents.map((eventItem, index) => (
            <div key={index} className="flex items-start space-x-4">
              <div className="text-[12px] font-[Inter] text-[#1F1D1D] font-normal min-w-[80px] pl-[30px]">
                {eventItem.time.split("-")[0]}-<br />
                {eventItem.time.split("-")[1]}
              </div>
              <div className="h-7 w-[2px] bg-[#1F1D1D]"></div>
              <div className="flex-1">
                <div className="font-semibold text-[#1F1D1D] text-[14px]">
                  {eventItem.event}
                </div>
                <div className="text-[12px] text-[#1F1D1D] font-normal">
                  {eventItem.staff}
                </div>
              </div>
            </div>
