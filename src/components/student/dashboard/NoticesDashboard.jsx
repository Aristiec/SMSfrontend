...
const NoticesDashboard = () => {
  return (
    <>
      <div className="w-full max-w-sm h-auto shadow-md rounded-[8px] overflow-hidden">
        <div className="w-full h-[56px] bg-[#04203E] flex items-center px-4 gap-[10px] rounded-t-[8px]">
          <h2 className="text-[#FAFCFD] font-[Merriweather] font-bold text-[20px] leading-[24px]">
            Notices
          </h2>
        </div>

        <div className="w-full max-h-[420px] p-4 rounded-b-lg bg-[#FAFCFD] overflow-y-auto">
          <div className="flex flex-col gap-5">
            {noticesData.map((notice) => (
              <div
                key={notice.id}
                className="w-full p-[12px] gap-[8px] rounded-[12px] bg-[#F4F7FA] flex flex-col"
              >
                {/*  Icon  */}
